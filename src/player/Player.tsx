...
const pickHls = (mod: unknown): HlsConstructorShape | null => {
  ...
  const selected = (candidate.default as unknown) ?? candidate.Hls ?? mod;
  if (typeof selected === 'function' || typeof selected === 'object') {
    return selected as HlsConstructorShape;
  }
  return null;
};
  ...
  useEffect(() => {
        ...
        if (
          ...
          if (autoPlay ?? autoplaySetting) {
            instance.on?.(HlsCtor.Events?.MANIFEST_PARSED ?? 'manifestParsed', () => {
              video
                .play()
                .catch((error: unknown) => console.warn('Autoplay failed', error));
            });
          }
          setEngine('hls');
          cleanup = () => {
    ...
    const setup = async () => {
      ...
      try {
        video.src = url;
        if (autoPlay ?? autoplaySetting) {
          void video.play().catch((error) => console.warn('Autoplay failed', error));
        }
        setEngine('native');
        cleanup = () => {
          video.pause();
